...
st.write(
    "Generate stylized fashion dolls in construction settings with toy blasters using the Google GenAI SDK."
)
...
if st.button("Generate Baddie Image", type="primary"):
  if not api_key:
    ...
    try:
      with st.spinner("Connecting to Google's remote brain... Painting pixels..."):
        # Initialize the Google GenAI client
        client = genai.Client(api_key=api_key)

        # Request image generation using the multimodal flash model endpoint
        ...
        image_found = False
        # Loop through the parts to locate and render the generated image
        for part in response.parts:
          if part.inline_data:
            ...
            st.image(
                image,
                caption="Y2K Construction Baddie",
